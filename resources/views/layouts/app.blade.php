...
<body class="bg-[#FBFBFA] dark:bg-[#0E0E0E] text-[#111111] dark:text-[#EDEDEC] antialiased min-h-screen flex flex-col font-sans selection:bg-[#EAEAEA]">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 w-full font-mono text-xs">
        @if (session('success'))
            <div class="flex items-center justify-between p-3.5 mb-4 rounded-[6px] bg-[#EDF3EC] text-[#346538] border border-[#346538]/20" role="alert">
                <div class="flex items-center space-x-2">
                    <span class="font-bold">[SUKSES]</span>
                    <span class="font-sans text-xs">{{ session('success') }}</span>
                </div>
                <button type="button" onclick="this.parentElement.remove()" class="text-[#346538] hover:opacity-75 text-sm">&times;</button>
            </div>
        @endif

        @if (session('error') || $errors->any())
            <div class="p-3.5 mb-4 rounded-[6px] bg-[#FDEBEC] text-[#9F2F2D] border border-[#9F2F2D]/20" role="alert">
                ...
                @if ($errors->any())
                    <ul class="list-disc list-inside text-[11px] space-y-0.5 text-[#9F2F2D]">
